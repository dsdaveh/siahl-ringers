...
Winter 2023 only: Let's skip the Mermen, since they appear to have gone nuts, but it's a small league and there were no ringer affects in the summer. The next two teams are from 6A, so it illustrates the main point better.\
\

```{r}
worst_offender <- ringer_effect_agg %>% slice(2) %>% pull(team, Division)
worst_offender
```
