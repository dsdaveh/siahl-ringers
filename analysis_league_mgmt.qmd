...
Now let's take a look the scoring for the game: It's arranged here with goals, primary and secondary assists on left for the visiting team and on the right for the home team. Goals that involved a player identified as a ringer are highlighted in yellow, and an asterisk (\*) in the `adj` column.
...
The league already has a policy against players from playing on teams that span more than 2 division levels. Unfortunately, this rule hasn't been enforced, presumably because it's difficult identify the ringers in time to prevent them from playing.
...
* Enter a game number 
    ...
    * (I plan to expand the UI to make games easier to find)
...
The practical application would be to allow captains, at any point in an active game, to protest an opposing player that should be disqualified to play at that level due to being on a roster in a more than 3 division-levels removed from the current game.

Putting such a policy in place, would be a great first step to cleansing the league from the affects of players who corrupt the fairness principles inherent to sports, and would encourage novice players that who endeavour to learn the great sport of hockey as adults.
