...
```{r}
set_seed <- codename(seed = 12)
gen_player_codename <- function() {
    codename(type = "gods") %>% snakecase::to_title_case() 
}

player_codename_map <- players %>% group_by(Name) %>% summarise(codename = gen_player_codename())
