...
home_games <- home_games[182:200]
games_adj <- tibble()
for (game_id in home_games) {
    scorecard <- scorecards[[game_id]]
    game_row <- games %>% 
        filter(Game == game_id, home == 1 ) %>% 
        mutate(v_goals = scorecard %>% scorecard_goals(home = TRUE) %>% nrow(),
               h_goals = scorecard %>% scorecard_goals(home = FALSE) %>% nrow(),
               vg_adj = scorecard %>% scorecard_goals(home = TRUE, remove_ringer_goals = TRUE) %>% nrow(),
               hg_adj = scorecard %>% scorecard_goals(home = FALSE, remove_ringer_goals = TRUE) %>% nrow())
